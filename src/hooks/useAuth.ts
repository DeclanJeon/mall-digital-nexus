...
interface AuthHook {
  ...
  isLoading: boolean;
  loginWithOtp: (email: string, otp: string, generatedOtp: string) => Promise<boolean>;
  logout: () => void;
...
export const useAuth = (): AuthHook => {
  ...
  const sendOtp = useCallback(async (email: string): Promise<{ success: boolean, otp?: string }> => {
    setIsLoading(true);
    try {
  ...
  const loginWithOtp = useCallback(async (email: string, otpValue: string, generatedOtp: string): Promise<boolean> => {
    ...
    try {
      ...
      if (loginRes.success) {
        ...
        if (normalizedEmail !== ADMIN_EMAIL && loginRes.accessToken && loginRes.refreshToken) {
        ...
        toast({
          title: "로그인 성공",
          description: normalizedEmail === ADMIN_EMAIL
            ? "관리자 계정으로 로그인되었습니다."
            : "인증이 완료되어 로그인되었습니다.",
        });
        setIsLoading(false);
        ...
        toast({
          title: "로그인 실패",
          description: "서버와의 통신에 실패했거나 인증 정보가 올바르지 않습니다.",
          variant: "destructive",
        });
        setIsLoading(false);
        return false;
