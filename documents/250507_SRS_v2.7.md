...
[3.1.8 피어몰 목록 (탐색 페이지) 21](https://docs.google.com/document/d/1mPRBjuNETFyrqLnRbFxtvqmUm9VEVytv/edit#heading=)
...
[3.3.3 정보 소스 통합 (피어허브 기능) 33](https://docs.google.com/document/d/1mPRBjuNETFyrqLnRbFxtvqmUm9VEVytv/edit#heading=)
...
[3.8.2 광고 캠페인 생성 및 설정 37](https://docs.google.com/document/d/1mPRBjuNETFyrqLnRbFxtvqmUm9VEVytv/edit#heading=)
...
[3.9.14 지식 공유 및 교육 인프라 지원 42](https://docs.google.com/document/d/1mPRBjuNETFyrqLnRbFxtvqmUm9VEVytv/edit#heading=)
...
[7.2 테스트 접근 방식 50](https://docs.google.com/document/d/1mPRBjuNETFyrqLnRbFxtvqmUm9VEVytv/edit#heading=)
...
[12.3 컴포넌트 시스템 (Tailwind CSS + Shadcn UI 기반) 59](https://docs.google.com/document/d/1mPRBjuNETFyrqLnRbFxtvqmUm9VEVytv/edit#heading=)
...
- 사용자 참여 유도: 지속적 자극(새로운 퀘스트, 이벤트 알림), 명확한 피드백(포인트 획득, 뱃지 부여 시각 효과), 사회적 상호작용(커뮤니티, 협력 퀘스트), 적절한 난이도 조절(개인 맞춤 퀘스트 추천). 광고 참여 유도 섹션.
...
- 4단계: 광고 타겟 설정: 인구통계, 지역(반경, 내 주변), 관심사/행동, 활동 시간, 관계 기반(팔로워, 뱃지 보유자, 7x7 신뢰 그룹 타겟팅 가능성 검토), 맞춤 타겟, 제외 타겟. 타겟 조합 저장/불러오기. 예상 도달 규모 표시. 시청자 선호도 조사 설정(선택적).
...
- 모듈식 아키텍처: MSA 적극 고려 (사용자, 피어몰, 상품, 커뮤니티, 광고, 추천, 알림, 실시간 통신, 히스토리 등).
...
- 백업 및 복구 메커니즘 (자가 호스팅/인프라 서비스 제공).
...
- 백엔드 (Server-Side): Node.js (NestJS 권장), API Gateway, 각 마이크로서비스 (사용자, 피어몰, 상품, 커뮤니티, 광고, 추천, 게임화, 알림, 실시간 통신(TIE/VI 관리), 히스토리 관리 등).
...
- 히스토리 로깅: 인프라 서비스 내 주요 액션(콘텐츠 CRUD, 구매, 커뮤니티 활동, 관계 변경, 설정 변경, 퀘스트/뱃지 변경 등) 발생 시, 관련 서비스는 히스토리 관리 서비스로 로그 데이터를 비동기적(메시지 큐 활용)으로 전송.
...
- P0 (Highest): 핵심 기능 & 보안: 가입/로그인/인증(추천인 포함), 피어몰 생성(패밀리 멤버십 선택 포함)/기본 관리, 상품 등록/구매/결제(가능시), QR 생성/스캔, 핵심 커뮤니티, 광고 캠페인 생성/집행/결제, 주요 보안(인증/인가/데이터 노출). (수정됨)
    

- P1 (High): 주요 기능 & 신뢰/게임화: 상품/콘텐츠/피어몰 검색/상세보기, 리뷰, 알림, 관계 설정(패밀리 멤버십 포함), 광고 타겟팅/성과 리포팅, 추천인 관리, 퀘스트/뱃지 기본 기능, TIE/VI 기본 연결. (수정됨)
...
- 특화 모니터링: 광고 시스템(서버 성능, 사기 의심), 블록체인(노드 상태, TPS), 게임화/신뢰 시스템(어뷰징 지표), 실시간 통신 서버(세션 수, 지연 시간). (수정됨)
...
(1.3 정의 및 약어 섹션의 모든 용어 포함 - 패밀리 멤버십 관련 용어 업데이트 반영됨)

## 9.2 관련 문서 참조

(1.4 참조 문서 섹션의 모든 문서 포함 - 클솔 패밀리 멤버십 정책 문서명 변경됨)
